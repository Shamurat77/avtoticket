...
// 1. ALGORITM UCHUN YO'NALISHLAR (Trassalar)
const highways = [
...
const Home = () => {
  ...
  // YANGI: Chiroyli bildirishnoma (Toast) uchun holat
  ...
  const handleSearch = (e) => {
    ...
    // Alertlar o'rniga yangi chiroyli bildirishnomani chaqiramiz
    if (!from || !to || !date) {
    ...
    const matchedRoutes = availableRoutes.filter(route => {
      let isMatch = false;

      for (let hw of highways) {
        const uFromIdx = hw.indexOf(from);
        const uToIdx = hw.indexOf(to);
      ...
      if (!isMatch && route.from === from && route.to === to) isMatch = true;
      return isMatch;
    });

    if (matchedRoutes.length > 0) {
      navigate('/seats', { state: { route: matchedRoutes[0], date: date, searchFrom: from, searchTo: to } });
    } else {
      showNotification(`Kechirasiz, ${from} dan ${to} ga qatnov topilmadi.`);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-70px)] bg-gray-50 flex flex-col items-center pt-10 md:pt-20 px-4 relative overflow-hidden">
      ...
      {/* ========================================= */}
      {/* YANGI: CHIROYLI TOAST BILDIRISHNOMA       */}
      {/* ========================================= */}
      <div className={`fixed top-24 right-5 z-100 transition-all duration-500 transform ${toast.show ? 'translate-x-0 opacity-100' : 'translate-x-32 opacity-0 pointer-events-none'}`}>
        <div className="flex items-center bg-white border-l-4 border-red-500 p-4 rounded-xl shadow-2xl min-w-75">
          ...
          <button onClick={() => setToast({show: false, message: '', type: 'error'})} className="ml-4 text-gray-400 hover:text-gray-800 transition">
            ✕
          </button>
        </div>
      </div>
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl md:text-5xl font-black text-blue-700 mb-4 tracking-tight">AvtoTicket orqali oson sayohat</h1>
        <p className="text-lg text-gray-500 font-medium">O'zbekiston bo'ylab avtobus chiptalarini onlayn xarid qiling</p>
      </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl w-full max-w-5xl border border-gray-100 z-10">
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="w-full md:w-1/3">
            <label className="block text-sm font-bold text-gray-700 mb-2">Qayerdan</label>
            <div 
              onClick={() => openModal('from')}
              className={`w-full px-4 py-3.5 border rounded-xl cursor-pointer flex justify-between items-center transition ${from ? 'border-blue-500 bg-blue-50/30' : 'border-gray-300 bg-white hover:bg-gray-50'}`}
            >
              <span className={from ? 'text-gray-900 font-medium' : 'text-gray-400'}>
          ...
          <div className="w-full md:w-1/3">
            <label className="block text-sm font-bold text-gray-700 mb-2">Qayerga</label>
            <div 
              onClick={() => openModal('to')}
              className={`w-full px-4 py-3.5 border rounded-xl cursor-pointer flex justify-between items-center transition ${to ? 'border-blue-500 bg-blue-50/30' : 'border-gray-300 bg-white hover:bg-gray-50'}`}
            >
              <span className={to ? 'text-gray-900 font-medium' : 'text-gray-400'}>
          ...
          <div className="w-full md:w-1/4">
            <label className="block text-sm font-bold text-gray-700 mb-2">Sana</label>
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-4 py-3.5 border border-gray-300 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required />
          </div>

          <div className="w-full md:w-auto mt-4 md:mt-0">
            <button type="submit" className="w-full md:w-auto px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-md">
            ...
            <div className="p-5 border-b flex justify-between items-center bg-gray-50">
              <div className="flex items-center gap-3">
                {activeRegion ? (
                  ...
                  <h3 className="text-xl font-bold text-gray-800">Hududni tanlang</h3>
                )}
              </div>
              <button 
